    ...
    */
    ...
    /**
        ...
        res:
            userLikes: Array(post), limit 10
    */
    app.get(`${serverConfig.BASE_URL}/likedSavedPosts/:pageNumber`, cors(), auth.validateToken, async (req, res) => {
        try {
            const pageNumber = req.params.pageNumber;
            const userId = mongoose.Types.ObjectId(req.user);
            const response = await Account.findById(userId, { totalSavedPosts: 1 });
            ...
            Account.aggregate([
                ...
                if (result) {
                    let likes = [];
                    await result[0].savedPosts.map((singlePost) => {
                        if (singlePost.userLikes[0] !== undefined) {
                            singlePost.userLikes.map((item) => {
                                if (item == req.user) {
                                    likes.push(singlePost._id)
                                }
                            })
                        }
                    })
                    res.status(200).json({ likedSavedPosts: likes });
                }
                if (err) {
                    console.log(err)
                    res.status(404).send('Saved posts not found');
